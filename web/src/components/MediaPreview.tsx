...
export function MediaPreview({
  ...
  const [loadedInfo, setLoadedInfo] = useState<FileEntry | null>(
    initialFileInfo ?? null,
  );
  const [infoAttempted, setInfoAttempted] = useState(
    !loadFileInfo || Boolean(initialFileInfo),
  );
  const [session, setSession] = useState(() => createPreviewSession());
  const [fallbackReason, setFallbackReason] = useState<string | null>(null);
  const [playerError, setPlayerError] = useState<string | null>(null);
  const [playerErrorMode, setPlayerErrorMode] = useState<
    MediaDecision["mode"] | null
  >(null);
  const [status, setStatus] = useState<PreviewStatus | null>(null);
  const [diagnosticsOpen, setDiagnosticsOpen] = useState(false);
  const [timelineState, setTimelineState] =
    useState<PlayerTimelineState>("unknown");
  ...
  if (!infoAttempted) {
    return (
      <div data-preview-no-close style={panelStyle}>
        <div
          className="shimmer"
          style={{ width: 420, maxWidth: "80vw", height: 220, borderRadius: 8 }}
        />
  ...
  const currentSourceFailed =
    Boolean(playerError && playerErrorMode === decision.mode) ||
    status?.state === "failed" ||
...
function VideoJsPlayer({
  ...
  useEffect(() => {
    ...
    const updateTimelineState = () => {
      ...
      const durationValue =
        typeof duration === "number" ? duration : Number.NaN;
      const seekable = player.seekable();
      const hasSeekableRange =
        seekable.length > 0 &&
        Number.isFinite(seekable.end(seekable.length - 1));
      const liveTracker = (
        player as unknown as { liveTracker?: { isLive?: () => boolean } }
      ).liveTracker;
...
function chooseMediaSource({
  ...
  const nativeSupport = nativeMediaSupport(
    entry,
    kind,
    sourceType,
    sourceTypeWithCodecs,
  );
  ...
  if (kind === "audio") {
    ...
    return {
      ...
      label: "No preview",
      reason:
        "Browser cannot play this audio source and transcoding is unavailable.",
      debug,
    };
  }
  ...
  if (!nativeSupport) {
    ...
    return {
      ...
      label: "No preview",
      reason:
        "Browser cannot play this video source and transcoding is unavailable.",
      debug,
    };
  }
...
function previewContentType(kind: MediaKind): string {
  return kind === "audio"
    ? PREVIEW_AUDIO_CONTENT_TYPE
    : PREVIEW_VIDEO_CONTENT_TYPE;
}
...
function nativeMediaSupport(
  ...
  const browserFriendlyContainer =
    ext === "mp4" || ext === "m4v" || ext === "webm" || ext === "mov";
...
function sourceContentType(
  ...
  const codecs = [
    mediaInfo?.video_mime_codec,
    mediaInfo?.audio_mime_codec,
  ].filter(Boolean);
