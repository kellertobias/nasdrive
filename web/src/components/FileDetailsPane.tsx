...
import { formatFileSize, formatModifiedDate, getFileIcon, hasThumbnail } from '../lib/icons';
...
function DetailSection({ title, details }: { title: string; details: Array<{ label: string; value: string }> }) {
  return (
    <section style={{
...
function getImageInfoDetails(info: NonNullable<FileEntry['image_info']>) {
  ...
  if (exif.DateTimeOriginal || exif.DateTime) {
    details.push({ label: 'Captured', value: exif.DateTimeOriginal || exif.DateTime });
  }
  if (exif.Orientation) details.push({ label: 'Orientation', value: exif.Orientation });
